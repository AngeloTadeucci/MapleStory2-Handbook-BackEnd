...
const evidence = 'NifToGltf/obj/outfit-studio-20260908/appearance-svg';
...
async function equip(name, id) {
 await slot(name).click(); await page.getByRole('searchbox').fill(String(id));
...
try {
 ...
 await check('Hair, Makeup and Face use three distinct SVG drawings', await page.locator('.appearance-controls').evaluate(el => {
   const icons = [...el.querySelectorAll('svg')];
   return icons.length === 3 && new Set(icons.map(icon => icon.innerHTML)).size === 3 && !el.querySelector('img') && icons.every(icon => icon.getAttribute('viewBox') === '0 0 48 48');
 }));
 async function centered() { return page.locator('.appearance-controls').evaluate(el => {
   ...
   return Math.abs(group.x + group.width / 2 - preview.x - preview.width / 2) < 1 && group.top > preview.top && group.bottom < preview.bottom;
 }); }
 await check('appearance buttons are horizontally centered inside the preview', await centered());
 await check('SVG icons inherit the Handbook text color', await page.locator('.appearance-controls svg').first().evaluate(el => getComputedStyle(el).color === 'rgb(222, 222, 222)'));
 ...
 for (const [name,id] of [['Hair',10200070],['Makeup',10400011],['Face',10300003]]) {
  ...
  await check(name + ' opens its picker and retains its distinct SVG after equipping', await slot(name).locator('svg').count() === 1 && await slot(name).locator('img').count() === 0);
 }
 ...
 await check('removing makeup retains the SVG control', await slot('Makeup').getAttribute('aria-label') === 'Makeup: Empty' && await slot('Makeup').locator('svg').count() === 1);
 ...
 await check('mobile keeps the appearance buttons centered inside the preview', await centered());
