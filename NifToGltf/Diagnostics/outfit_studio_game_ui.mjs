...
const evidence = 'NifToGltf/obj/outfit-studio-20260908/game-ui-02';
...
page.on('crash', () => { errors.push('Chromium page crashed'); console.log('CRASH'); });
page.on('console', message => { if (message.text().startsWith('QA:')) { trace.push(message.text()); console.log(message.text()); } });
...
try {
  ...
  await scene().evaluate(el => {
    const prototype = Object.getPrototypeOf(el.outfitViewer);
    for (const name of ['setBody','equipBundle','setBackground','screenshot','mount']) {
      const original = prototype[name];
      prototype[name] = function (...args) {
        console.log('QA: start ' + name);
        const result = original.apply(this, args);
        if (result instanceof Promise) return result.then(value => { console.log('QA: done ' + name); return value; }, error => { console.log('QA: failed ' + name + ': ' + error.message); throw error; });
        console.log('QA: done ' + name); return result;
      };
    }
  });
  await expect.poll(() => page.locator('.item-card').count()).toBeGreaterThan(0);
  await check('14 slots, without pendants, rings, belts or ears; earrings retained', await page.locator('.slot-rail .equipment-slot').count() === 11 && await page.locator('.appearance-slot').count() === 3 && await page.getByRole('button', { name: /^(Pendants|Rings|Belts|Ears):/ }).count() === 0 && await page.getByRole('button', { name: /^Earrings:/ }).count() === 1);
  ...
  await check('canvas uses the original DDS backdrop', await scene().evaluate(el => el.outfitViewer.scene.background.image.src.endsWith('/outfits/character-background.png')));
  await check('all eleven equipment slots load original atlas tiles', await page.locator('.slot-rail .equipment-slot img').evaluateAll(images => images.length === 11 && images.every(image => image.complete && image.naturalWidth === 68 && image.src.includes('/outfits/slots/'))));
  await check('slot order follows the reference with pendant omitted', JSON.stringify(await page.locator('.left-rail button').evaluateAll(nodes => nodes.map(n => n.getAttribute('aria-label').split(':')[0]))) === JSON.stringify(['Hats','Eyewear','Tops','Pants','Back','Right hand']) && JSON.stringify(await page.locator('.right-rail button').evaluateAll(nodes => nodes.map(n => n.getAttribute('aria-label').split(':')[0]))) === JSON.stringify(['Earrings','Face accessories','Gloves','Shoes','Left hand']));
  await page.locator('.fitting-room').screenshot({ path: evidence + '/empty-slots.png' });
  ...
  await check('equipped outfit uses its rarity frame and outfit marker', await page.getByRole('button', { name: /^Tops:/ }).locator('.outfit-marker').count() === 1 && await page.getByRole('button', { name: /^Tops:/ }).locator('.worn-background').getAttribute('src') === await scene().evaluate(el => '/resource/sprites/slot bg ' + (el.outfitViewer.equippedItems.find(b => b.slots.includes('CL')).item.rarity ?? 1) + '.png'));
  await check('full outfit repeats a dimmed icon in pants', await page.getByRole('button', { name: /^Pants:/ }).locator('.linked-copy').evaluate(el => getComputedStyle(el).filter === 'brightness(0.5)'));
  ...
  await page.getByRole('button', { name: 'Export current outfit', exact: true }).click();
  await check('import restores the DDS studio background', await scene().evaluate(el => el.outfitViewer.scene.background.image.src.endsWith('/outfits/character-background.png')));
  ...
  await expect.poll(() => scene().evaluate(el => el.outfitViewer.scene.background.image.src.includes('bg_henesys_a'))).toBe(true);
  await page.getByLabel('Background', { exact: true }).selectOption('');
  await expect.poll(() => scene().evaluate(el => el.outfitViewer.scene.background.image.src.endsWith('/outfits/character-background.png'))).toBe(true);
  await check('resetting scenery restores original studio artwork', true);
  await page.locator('.studio-settings summary').click();
  ...
  for (const angle of ['Front','Side','Back']) {
    ...
    await page.locator('.fitting-room').screenshot({ path: evidence + '/outfit-' + angle.toLowerCase() + '.png' });
  }
...
} catch (error) { errors.push(error.stack ?? String(error)); process.exitCode = 1; await page.screenshot({ path: evidence + '/failure.png', timeout: 10000 }).catch(() => {}); }
finally { writeFileSync(evidence + '/report.json', JSON.stringify({ checks, errors, trace }, null, 2)); console.log(JSON.stringify({ checks: checks.length, errors }, null, 2)); await browser.close(); }
