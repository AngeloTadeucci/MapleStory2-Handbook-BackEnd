...
internal sealed record NifSkin(int Root, Matrix4x4 Transform, int[] Bones, Matrix4x4[] BindTransforms);
...
internal sealed class NifDocument {
    ...
    public NifSkin Skin(int block) {
        ...
        return new NifSkin(root, transform, bones, binds);
    }
    public static NifDocument Load(string path) {
        byte[] data = File.ReadAllBytes(path);
        int line = Array.IndexOf(data, (byte) '\n');
        if (line < 0 || Encoding.ASCII.GetString(data, 0, line) != "Gamebryo File Format, Version 30.2.0.3") {
            throw new NotSupportedException($"{path}: expected NIF 30.2.0.3.");
        }
        NifReader r = new(data);
        r.Take(line + 1);
        if (r.U32() != 0x1E020003 || r.Byte() != 1) throw new NotSupportedException("Unsupported NIF version/endian.");
        r.U32();
        ...
        for (int i = 0; i < blocks.Length; i++) {
            if (blocks[i].Type is not ("NiNode" or "NiMesh")) continue;
            try {
    ...
    private NifNode ReadNode(int id) {
        ...
        int[] children = [];
        NifMesh? mesh = null;
        if (Blocks[id].Type == "NiNode") {
            children = r.Refs().Where(child => child >= 0).ToArray();
            r.Refs(); // Effects have a separate representation from static geometry.
        } else {
            int materials = r.Count(8);
            for (int i = 0; i < materials; i++) Name(r);
            r.Take(materials * 4);
            r.I32();
            r.Bool();
